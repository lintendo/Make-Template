    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-[#D6D1C7]">
          <h2 className="text-xl font-serif text-[#2C2A26]">你的购物袋 ({items.length})</h2>
          <button 
            onClick={onClose} 
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center space-y-4 opacity-60">
              ...
              <p className="font-light text-[#5D5A53]">购物袋还是空的。</p>
            </div>
          ) : (
            items.map((item, idx) => (
              <div key={`${item.id}-${idx}`} className="flex gap-4 animate-fade-in-up">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-xs text-[#A8A29E] uppercase tracking-widest mt-1">{CATEGORY_LABELS[item.category]}</p>
                  </div>
                  <button 
        ...
        <div className="p-6 border-t border-[#D6D1C7] bg-[#EBE7DE]/30">
          <div className="flex justify-between items-center mb-6">
            <span className="text-sm font-medium uppercase tracking-widest text-[#5D5A53]">小计</span>
            <span className="text-xl font-serif text-[#2C2A26]">${total}</span>
          </div>
          <p className="text-xs text-[#A8A29E] mb-6 text-center">配送与税费将在结账时计算。</p>
          <button 
            onClick={onCheckout}
