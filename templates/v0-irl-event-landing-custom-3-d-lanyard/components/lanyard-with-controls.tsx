...
import darkCardBase from "../public/card-base-dark.png";
import lightCardBase from "../public/card-base-light.png";
...
const darkCardBaseSrc = darkCardBase as unknown as string;
const lightCardBaseSrc = lightCardBase as unknown as string;
...
function StaticLanyardPreview({
  ...
  const imageSrc = cardTextureUrl || (variant === "dark" ? darkCardBaseSrc : lightCardBaseSrc);

  return (
    <div
      className={clsx(
        containerClassName ||
          "relative z-0 flex h-screen w-full origin-center scale-100 items-center justify-center",
        "lanyard-static-shell"
      )}
      aria-hidden="true"
    >
      <div className="lanyard-static-scene">
        <div className="lanyard-static-strap" />
        <div className="lanyard-static-clip" />
        <div className="lanyard-static-card-wrap">
          <img className="lanyard-static-card" src={imageSrc} alt="" />
        </div>
      </div>
    </div>
  );
...
export default function LanyardWithControls({
  ...
  const getShareableUrl = useCallback(() => {
    ...
    if (appliedName) {
      ...
      return `${baseUrl}#page=lanyard&u=${encrypted}`;
    }
    return `${baseUrl}#page=lanyard`;
  }, [appliedName, appliedVariant]);

  // Share message templates
  const shareMessage = appliedName
    ? `我会参加 @v0 Prompt to Production 纽约！来看看我的专属挂绳名牌`
    : `来看看 v0 IRL 纽约！创建你的专属活动挂绳名牌`;
