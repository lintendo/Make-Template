...
import React, { useMemo, useState, useCallback, useEffect, useRef, createContext, useContext, type ReactNode } from "react"
...
export function SplitFlapMuteToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      aria-label={audio.isMuted ? "开启音效" : "关闭音效"}
    >
      {audio.isMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
      <span>{audio.isMuted ? "声音关闭" : "声音开启"}</span>
    </button>
  )
}
...
function SplitFlapChar({ char, index, animationKey, skipEntrance, speed, playClick }: SplitFlapCharProps) {
  ...
  const bgColor = isSettled ? "oklch(0.09 0 0)" : "oklch(0.7 0.2 45 / 0.2)"
  const textColor = isSettled ? "oklch(0.96 0 0)" : "oklch(0.7 0.2 45)"
