...
**Card Tap → Expand**
- On tap, all other cards animate downward into the pocket simultaneously (`ease-in-out web transition (preserve the source timing)`, 0.4s)
- The tapped card grows from its peek state to full card size (`spring-style web motion (response: 0.5, damping: 0.8)`)
- Below the expanded card, the supplementary content (transactions, balance, actions) cross-fades in at +200ms delay
- Tap feedback state: `medium-emphasis confirmation state` on the card; `selection-changed state` if scrolling reveals a new card

**Pull to Collapse**
- Drag down on the expanded card to collapse it back into the stack
- Drag threshold: 60pt
- Rubber-band resistance after 100pt
- visible feedback state on release: `soft-emphasis confirmation state`
...
- Disclosure indicator: 13pt `chevron.right` web-accessible icon `#636368`, right-aligned
...
- Height: 56pt
- Background: `translucent backdrop blur` blur over `#000000`
- Buttons: 3 horizontally arranged, each 44pt × 44pt circular container `rgba(255,255,255,0.12)` background
- Glyph: web-accessible icon white 18pt — `creditcard.fill`, `list.bullet.rectangle`, `ellipsis`
...
- Glyph: 16pt SF Pro Display weight Bold `+` white centered, or `plus` web-accessible icon
- Tap: `soft-emphasis confirmation state` visible feedback state, opens a sheet for "Add Card"
...
| Sheet (Level 3) | `rgba(0,0,0,0.5) 0 -8px 32px` + `translucent backdrop blur` over `#000000` | Detail sheets, action toolbars |
...
### Motion
- **Card expand**: spring `response: 0.5, damping: 0.8` — slightly bouncy but contained; the card grows from 80pt peek to full 220pt body in ~500ms; rest of stack tucks down `ease-in-out web transition (preserve the source timing)` 0.4s in parallel
- **Card collapse**: same physics reversed; visible feedback state on release `soft-emphasis confirmation state`
- **Card stack scroll**: standard iOS scroll physics with overshoot rubber-band; visible feedback state `selection-changed state` every 80pt
- **Double-side-button → Apple Pay**: card slams to center, screen turns true black, Face ID glyph fades in, success → green check, visible feedback state `success confirmation state`
...
- **Boarding pass flip**: tap the QR code to flip the card 180° around the Y-axis with a 600ms `ease-in-out web transition (preserve the source timing)` for the gate-agent scanner
...
- Use `medium-emphasis confirmation state` visible feedback state when expanding a card; `.soft` when collapsing
...
- "Animate the card expand: on tap of a card in the stack, the tapped card grows from its 80pt peek to its full 220pt body using responsive React/Web `spring-style web motion (response: 0.5, damping: 0.8)`. All other cards translate downward into the pocket using `ease-in-out web transition (duration: 0.4)`. After 200ms the supplementary content (balance, transactions, action toolbar) cross-fades in beneath the expanded card. visible feedback state: `visible feedback state` on tap."
- "Design the Apple Pay invocation: when the user double-clicks the side button, the foreground card slams to center of the screen at full size, the rest of the UI fades to `#000000` true black, and below the card a Face ID glyph (rounded square with viewfinder corners, 64pt, white) appears with 'Use Face ID to confirm' in SF Pro Text 17pt Regular white centered. On success, a green check `#30D158` 64pt circle replaces the Face ID glyph, visible feedback state `success confirmation state`, then 'Hold near reader' in SF Pro Display 22pt Semibold white at the bottom."
