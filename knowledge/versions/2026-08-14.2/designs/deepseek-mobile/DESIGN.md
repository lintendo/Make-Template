...
### Motion
- **Toggle on/off**: fill + border + content color crossfade over 150ms ease; visible feedback state on engage
...
- **visible feedback state**: soft confirmation state on toggle engage and send; visible feedback state on copy
...
- "Build the DeepSeek reasoning trace in responsive React/Web: a recessed panel, background `#16171A` (darker than the `#0E0E10` canvas), a 2pt `#4D6BFE` left bar, 10pt corner radius. Header row: brain web-accessible icon 14pt `#4D6BFE` + 'Thought for 14 seconds' (Inter 12pt w600 `#9A9BA0`) + '· tap to collapse' (`#6A6B70`). Body: chain-of-thought in Inter 13pt w400 **italic** `#8A8B90`, line-height 1.55. Tap header → 200ms height collapse to a single line with a chevron rotation."
