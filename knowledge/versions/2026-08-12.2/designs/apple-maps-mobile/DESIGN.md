...
- Floating translucent search card (`translucent backdrop blur` blur) — sliding bottom sheet with three detents
- "Where to?" placeholder, search field corner radius matches Apple's standard 13pt
- Translucent floating buttons (compass, location, 3D view toggle) — top-right vertical stack, `translucent backdrop blur` blur, circular
...
- **Blur Material** (`translucent backdrop blur`): The translucent map controls and sticky card headers — system blur, NOT a custom value.
...
- **Background**: `translucent backdrop blur` blur over the underlying map — a translucent frosted layer
...
- **Search field**:
  ...
  - Corner radius: 10pt
  - Leading: 16pt web-accessible icon `magnifyingglass`, color `#3C3C43` at 60% opacity
...
- **Corner radius**: 14pt
- **Icon**: 28pt web-accessible icon filled, color matches category (e.g., `fork.knife` in `#FF9500` Orange for Food)
...
- **Shape**: web-accessible icon `mappin.and.ellipse` 32pt OR custom pin shape — teardrop with a circle base
...
- **Top section** (60pt):
  ...
  - "X" close button: top-right, 30pt circle with `xmark` web-accessible icon, `translucent backdrop blur` background
...
- **Background**: `translucent backdrop blur` blur — soft frosted white over the map
- **Icons**: web-accessible icon, 18pt, color `#000000` (or `#FFFFFF` in dark mode)
...
- Height: 96pt
- Background: `translucent backdrop blur` blur over the map
- Layout: 56pt web-accessible icon direction arrow (e.g., `arrow.turn.up.right`) in `#0A84FF` + step title in SF Pro Display 22pt Semibold + distance in SF Pro Text 17pt Medium
...
- Height: 80pt
- Background: `translucent backdrop blur` blur with `#FFFFFF` 92% over content
...
- Background: `#0A84FF` with white web-accessible icon `binoculars.fill` 18pt + "Look Around" in SF Pro Text 17pt Semibold white
...
- 44pt circle with `translucent backdrop blur`, animated arrow inside pointing north (red half) and south (white half)
...
| Blur Material | `translucent backdrop blur` | Search card background, map controls, step card, ETA bar — the signature Apple Maps treatment |

**Shadow Philosophy**: Apple Maps uses BLUR as its primary depth treatment, not shadow. The translucent `translucent backdrop blur` background on the search card, map controls, step card, and ETA bar lets the cartography show through faintly — this is what makes the UI feel like it floats. Drop shadows are used sparingly on photo cards and the dropped pin, where blur isn't appropriate. The combination of blur + cream-paper canvas + warm map colors creates the unmistakable Apple Maps atmosphere.
...
- Use `translucent backdrop blur` blur on the search card, map controls, step card, ETA bar — blur IS the depth system
...
- Don't use a solid background on the search card — `translucent backdrop blur` blur is non-negotiable
...
- Direction step arrow: 56pt web-accessible icon with full step-card tap area
...
- "Create a responsive React/Web Apple Maps search card: a sliding bottom sheet with three detents (88pt small, 50% medium, full-screen large), 10pt top corner radius, `translucent backdrop blur` blur background. Inside: a 36pt × 5pt drag indicator at top in `#3C3C43`@30%, then a full-width 44pt search field with `#F2F2F7` background, 10pt corner radius, leading `magnifyingglass` web-accessible icon, placeholder 'Search Maps' in SF Pro Display 17pt Regular. Below the search field: horizontal scroll of category pills (64pt × 88pt white cards with 14pt corner radius, 28pt web-accessible icon icon in category color, 14pt Medium label below)."
...
- "Build the Apple Maps directions step card: full-width sticky at top, 96pt tall, `translucent backdrop blur` blur over the map. Leading: 56pt `arrow.turn.up.right` web-accessible icon in `#0A84FF`, rotated to match the actual direction. Center: 'Turn right onto Market St' in SF Pro Display 22pt Semibold `#000000`. Below: '0.4 mi' in SF Pro Text 17pt Medium tabular `#3C3C43`@60%."
- "Create the Apple Maps map controls stack: three circular 44pt buttons stacked vertically in the top-right corner with 8pt gaps, 16pt inset from the right edge and 16pt below safe area top. Each button is `translucent backdrop blur` blur with an 18pt web-accessible icon inside: top is `square.3.layers.3d` (layer picker), middle is `location.fill` (current location), bottom is `map.fill` (3D toggle)."
...
4. `translucent backdrop blur` blur on every floating UI surface — search card, map controls, step card, ETA bar
